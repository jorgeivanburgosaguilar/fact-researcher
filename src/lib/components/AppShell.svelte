<script>
  ...
  // Initial props seed local review state; later parent values must not overwrite edits.
  ...
  async function importFile(file) {
    error = '';
    try {
      if (onimport) facts = await onimport(file);
      else {
...
{#if facts.length}<FactsWorkbench
    {facts}
    {filename}
    onreview={reviewFact}
    onexport={() => onexport(facts, filename)}
  />{:else}<FileImporter onfile={importFile} {error} />{/if}
